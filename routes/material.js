...
router.post('/insertmaterial', upload.single("picture"), function (req, res, next) {
    ...
    pool.query('insert into material (materialname,status, picture) values(?,?,?)', [req.body.materialname,req.body.status, req.body.myfilename], function (error, result) {
      if (error) {
...
router.post('/editmaterialpicture', upload.single("picture"), function (req, res, next) {
    console.log("BODY:", req.body)
    console.log("FILE:", req.file)
